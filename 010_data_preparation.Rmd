...
Need five different datasets to appropriately model the data.
...
# # Imputing LAI
# disturbed_mean_lai <- abiotic_quadrat.df %>% 
#   select(site, av_quad_lai) %>%
#   filter(site == "D") %>% 
#   distinct() %>%
#   summarise(mean = mean(av_quad_lai, na.rm = TRUE)) %>%
#   unlist(use.names = FALSE)
# 
# abiotic_quadrat.df %<>%
...
# Create community aggregated SLA
community.df <- domsla_rq1.df %>%
  ...
  mutate(sla_cover = dom_sla*dom_seedling_cover,
         sla_cover_mean = mean(sla_cover), 
         sla_cover_sd = sqrt(var(sla_cover))) %>%
  group_by(site, transect, quadrat) %>%
  mutate(sla_cw_mean = mean(sla_cover_mean), sla_cw_sd = sqrt(var(sla_cover_sd))) %>%
  ungroup()

communityagg_vars <- c("site", "transect", "quadrat", 
          "sla_cw_mean", "sla_cw_sd",
...
coefficient_variance_vars <- c("site", "transect", "quadrat", "dom_species_id",
                               "sla_mean", "sla_sd", "sla_cover_mean", "sla_cover_sd",
                               "dom_seedling_cover", "overall_seedling_cover", "scaled_dom_seedling_cover", "scaled_overall_seedling_cover",
                               "av_trans_decomposition", "max_temp", "av_quad_compaction", "av_quad_lai", "av_quad_gft")
...
hobo_rq2.df <- hobo3 %>%
  ...
  filter(date_time > as.POSIXct(strftime("2017-09-26 3:00:00", 
         ...
         date_time < as.POSIXct(strftime("2017-09-28 22:00:00", 
                                         "%Y-%m-%d %H:%M:%S"))) %>%
  select(site, transect, hobo_id, temp, rel_humid, av_trans_elevation, av_trans_compaction, av_trans_lai, av_trans_gft, av_trans_decomposition)
