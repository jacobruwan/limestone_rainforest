...
#overall_seedling_cover
g_pp <- ggplot(aes(y=dom_shannon, x=overall_seedling_cover, col=site), data=domshannons_rq1.df) + geom_point() + labs(title=" (b) Overall Seed Cover", y="Shannon Index", x="overall_seedling_cover (scaled)") + geom_smooth(method = "lm") + theme_project()+ scale_colour_manual(values = colours.site)
