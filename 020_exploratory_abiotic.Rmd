...
load(paste0(location, "abiotic_elevation.RData"))
...
```{r}
## Abiotic variables
abiotic_vars <- c("av_quad_compaction", "av_trans_decomposition", "min_temp", "max_temp", "range_temp", "av_trans_elevation", "av_quad_lai")


abiotic_quadrat.df %>% 
  select(av_trans_elevation, min_temp, max_temp, range_temp, 
         av_trans_decomposition, av_quad_compaction, av_quad_lai) %>%
  cor() %>%
  round(digits=2)

abiotic_quadrat.df %>% 
  select(av_trans_elevation, min_temp, max_temp, range_temp, 
         av_trans_decomposition, av_quad_compaction) %>%
  pairs(upper.panel = NULL)
title("Abiotic Indicator Correlations")
...
### Temperature Time Series Fit
```{r}
ts.df <- datarq2 %>%
  select(hobo_id, date_time, temp, site, av_trans_elevation, av_trans_decomposition, av_trans_compaction) %>%
  ts()
...
splinefit_el <- lm(temp ~ site + bs(time24, knots = c(k1, k2, k3)) + av_trans_elevation:bs(time24, knots = c(k1, k2, k3)) + av_trans_elevation,
          data = datarq2)

# summary(splinefit_el)
```
