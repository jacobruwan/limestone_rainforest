...
domsla_rq1.df %>%
  ...
  summarise(sla_group_abundance = sum(dom_abundance)) %>%
  ggplot(aes(x=sla_group, y = sla_group_abundance, fill = dom_species_id)) +
  ...
  xlab("Dominant Species Abundance") +
  ylab("SLA Group") +
...
```{r}
domsla_rq1.df %>%
  ggplot(aes(x = species_richness, y = dom_sla, 
             col = site)) +
  geom_point(size = 1) +
  theme_project() +
