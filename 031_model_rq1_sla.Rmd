...
```{r}
domsla_rq1.df %>% 
  filter(dom_species_id %in% c("1", "2", "3")) %>% 
  select(-species_id, -site, -transect, -quadrat) %>%
  distinct() %>%
  t()
...
## Abiotic variables
abiotic_vars <- c("av_quad_compaction", "av_trans_decomposition", "min_temp", "max_temp", "range_temp", "av_trans_elevation")
...
# correlation results
# elevation will account for decopmosition and min_temperature, however we will leave compaction in the model for now as it is at the transect level
# range_temp will account for max_temp

biotic_vars <- c("dom_sla", "dom_abundance", "dom_seedling_cover", "overall_seedling_cover", "dom_species_id",
...
predictors <- names(sla.df)[-(which(names(sla.df) %in% c("dom_sla_weighted_abundance", "transect", "quadrat",
                                                         "min_temp", "max_temp", "av_trans_decomposition",
...
save(Log_Likelihood_Summary, file = paste0(data_folder, "inla_mlik_rank.RData"))
```

#### Model: Minimum Marginal Log-Likelihood
```{r}
top10 <- Log_Likelihood_Summary %>%
...
inla_formula <- as.formula("dom_sla_weighted_abundance ~ site + dom_species_id + scaled_dom_seedling_cover + scaled_av_trans_elevation + site:dom_species_id + dom_species_id:scaled_dom_seedling_cover + f(transect, model='iid') + f(quadrat:transect, model = 'iid')")
...
fixed.df %>%
  ...
  geom_errorbarh() +
  theme_project() +
  ggtitle("Standardised Predictors: Parameter Estimate Credible Intervals") +
