...
sla.df <- domsla_rq1.df %>%
  ...
  select(-av_dom_leaf_dry_weight,-total_dom_leaf_dry_weight,
         ...
         -species_id, -total_dom_leaf_area, -av_dom_sla_from_ind, -av_dom_sla_from_total, -dom_sla, -dom_abundance, -dom_seedling_cover, -overall_seedling_cover) 
...
inla_formula <- as.formula("dom_sla_weighted ~ site + dom_species_id + scaled_dom_seedling_cover + scaled_overall_seedling_cover + f(transect, model='iid') + f(quadrat:transect, model = 'iid')")
...
sla.df %>%
  ggplot(aes(x = pred_dom_sla_weighted)) +
