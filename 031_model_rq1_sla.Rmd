...
sla.df <- domsla_rq1.df %>%
  ...
  mutate(dom_sla = dom_sla/max(dom_sla),
         dom_abundance = dom_abundance/max(dom_abundance))
...
sla.df %>%
  mutate(dom_sla_pred = summary(fit)$cpo$pit) %>%
  ggplot() +
  geom_point(aes(x = site, y = dom_sla_pred))
