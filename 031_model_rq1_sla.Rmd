...
biotic_vars <- c("dom_seedling_cover", "overall_seedling_cover", "dom_species_id", "dom_abundance")
...
predictors <- names(sla.df)[-(which(names(sla.df) %in% c("dom_sla_weighted_abundance", "transect", "quadrat",
                                                         "min_temp", "range_temp", "av_trans_decomposition",
...
#### Model: Minimum Marginal Log-Likelihood (No Interactions)
```{r}
...
$$
y_{sla} = \beta_0 + \beta_1x_e + \beta_2x_{id2} + \beta_3x_{id3} + \beta_4x_{cover} + \beta_5x_{siteP} + \beta_6x_{e:id2} + \beta_7x_{e:id3} + u_{transect} + v_{quadrat/transect} + \varepsilon_{residual}
...
realeffect.df %>%
  ...
  xlab("Real SLA Effect - Credible Intervals") +
